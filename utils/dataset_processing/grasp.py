...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1):
    ...
    """
    print("hello_local_max",q_img.shape)
    local_max = []
    local_max = peak_local_max(q_img, min_distance=20, threshold_abs=None, num_peaks=no_grasps)#找出了质量分布中的局部最大值# min_distance=20, threshold_abs=0.2
    print("local_max@@",local_max,len(local_max))
    grasps = []
    for grasp_point_array in local_max:
        ...
        g = Grasp(grasp_point, grasp_angle)#生成了抓取格式对象，每个g都是一个独立完整的抓取对象
        if width_img is not None:
            g.length = width_img[grasp_point]
            g.width = g.length/2

        grasps.append(g)#一个完整的抓取包括中心点，角度，抓取宽度，这里存了所有的抓取质量局部最大值的候选抓取
